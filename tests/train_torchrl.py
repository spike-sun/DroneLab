...
parser.add_argument("--use_fabric", action="store_true", default=True, help="Use fabric and disable USD I/O operations.")
...
from torchrl.envs import GymWrapper, TransformedEnv, StepCounter, VecNorm, IsaacGymWrapper, Compose
...
from envs import forest_chaser
...
value_net = MLP(input_size=58, output_size=1).to(device)

value_module = ValueOperator(
    module=value_net,
    in_keys=[("observation", "evader")],
)
...
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optim, total_frames // frames_per_batch * num_epochs, 0.0)

# TODO
# fix CUDA out of memory

writer = SummaryWriter(log_dir)
os.mkdir(f"{log_dir}/checkpoint")
pbar = tqdm(total=epoch)
max_reward = float("-inf")
for i, tensordict_data in enumerate(collector):
    for _ in range(num_epochs):
        with torch.no_grad():
            tensordict_data.detach_()
    ...
    writer.add_scalar('mean_reward', reward, i)
    writer.add_scalar('max_step', tensordict_data["step_count"].max().item(), i)
    if reward > max_reward:
        max_reward = reward
        torch.save(policy_module.state_dict(), f"{log_dir}/checkpoint/best_policy.pt")
        torch.save(env.transform.state_dict(), f"{log_dir}/checkpoint/best_transform.pt")
    torch.save(policy_module.state_dict(), f"{log_dir}/checkpoint/last_policy.pt")
    torch.save(env.transform.state_dict(), f"{log_dir}/checkpoint/last_transform.pt")
    pbar.update()