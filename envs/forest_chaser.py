...
class ForestChaserCfg(DirectRLEnvCfg):
    ...
    scene = SceneCfg()
    evader_policy = 'logs/policy/evader/PPO_2025-01-14_19-43-00/checkpoints/best_agent.pt'
...
class ForestChaser(DirectRLEnv):
    ...
    def __init__(self, cfg: ForestChaserCfg, render_mode: str | None = None, **kwargs):
        ...
        self.evader_observation = torch.zeros((self.num_envs, gym.spaces.flatdim(self.observation_space['evader'])), device=self.device)
        evader_action_space = Box(low=-1.0, high=1.0, shape=[3])
        self.evader_policy = EvaderPolicy(self.observation_space['evader'], evader_action_space, device=self.device).to(self.device)
        self.evader_state_preprocessor = RunningStandardScaler(size=58, device=self.device)
    ...
    def _pre_physics_step(self, actions):
        with torch.inference_mode():
            actions_e, _, _ = self.evader_policy.compute({'states': self.evader_state_preprocessor(self.evader_observation)}, role='')
    ...
    def _reset_idx(self, env_ids):
        ...
        root_state[:, 0] = -52.0 #+ (0.3 * torch.randn(len(env_ids), device=self.device)).clamp(-0.3, 0.3)
        root_state[:, 1] = new_y
        #root_state[:, 4:7] = (0.1 * torch.randn((len(env_ids), 3))).clamp(-0.1, 0.1)
        root_state[:, 7] = 0.5
    ...
    def _get_observations(self):
        ...
        self.evader_observation = FlattenObservation.flatten_observation(observations['evader'], self.observation_space['evader'])

        if ENABLE_CAMERAS:
            observations['chaser']['rgb'] = self.camera_c.data.output['rgb'][:,:,:,0:3]
            observations['chaser']['depth'] = self.camera_c.data.output['depth'][:,:,:,0]
